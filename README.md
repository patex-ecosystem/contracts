# Patex Smart Contracts

[![codecov](https://codecov.io/gh/patex-ecosystem/patex-network/branch/master/graph/badge.svg?token=0VTG7PG7YR&flag=contracts-tests)](https://codecov.io/gh/patex-ecosystem/patex-network)

`@eth-patex/contracts` contains the various Solidity smart contracts used within the Patex system.
Some of these contracts are [meant to be deployed to Ethereum ("Layer 1")](https://github.com/patex-ecosystem/patex-network/tree/develop/packages/contracts/contracts/L1), while others are [meant to be deployed to Patex ("Layer 2")](https://github.com/patex-ecosystem/patex-network/tree/develop/packages/contracts/contracts/L2).
Within each contract file you'll find the network upon which the contract is meant to be deloyed, listed as either `EVM` (for Ethereum) or `OVM` (for Patex).
If neither `EVM` nor `OVM` are listed, the contract is likely intended to be used on either network.

## Usage (npm)

You can import `@eth-patex/contracts` to use the Patex contracts within your own codebase.
Install via `npm` or `yarn`:

```shell
npm install @eth-patex/contracts
```

Within your contracts:

```solidity
import { SomeContract } from "@eth-patex/contracts/path/to/SomeContract.sol";
```

Note that the `/path/to/SomeContract.sol` is the path to the target contract within the [contracts folder](https://github.com/patex-ecosystem/patex-network/tree/develop/packages/contracts/contracts) inside of this package.
For example, the [L1CrossDomainMessenger](https://github.com/patex-ecosystem/patex-network/blob/develop/packages/contracts/contracts/L1/messaging/L1CrossDomainMessenger.sol) contract is located at `packages/contracts/contracts/L1/messaging/L1CrossDomainMessenger.sol`, relative to this README.
You would therefore import the contract as:


```solidity
import { L1CrossDomainMessenger } from "@eth-patex/contracts/L1/messaging/L1CrossDomainMessenger.sol";
```
...
Patex expects that certain contracts (called "predeploys") be deployed to the L2 network at pre-determined addresses.
...
Patex has removed the whitelist from the Patex mainnet.
